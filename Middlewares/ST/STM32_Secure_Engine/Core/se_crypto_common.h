...
/** @defgroup SE_CRYPTO_COMMON SE Crypto common
 *  @Common crypto elements
  * @{
  */
...
typedef struct
{
  ...
  uint8_t   *pTag;                 /*!< Pointer to Authentication TAG. This value must be set in decryption, and this TAG will be verified */
...
/** @defgroup SE_CRYPTO_COMMON_Public_Macros_AES_GCM AES GCM Public Macros
  ...
  * @{
  */
#define IS_SE_CRYPTO_AES_GCM_NONCE_SIZE(SIZE)   ((SIZE) == SE_NONCE_LEN)
#define IS_SE_CRYPTO_AES_GCM_TAG_SIZE(SIZE)     ((SIZE) == SE_TAG_LEN)
/**
  * @}
  */
...
/** @addtogroup SE_CRYPTO_COMMON SE Crypto common functions
  * @{
  */
...
SE_ErrorStatus SE_CRYPTO_AES_GCM_Encrypt_Append(const uint8_t *pInputBuffer, int32_t InputSize, uint8_t *pOutputBuffer, int32_t *pOutputSize);
...
SE_ErrorStatus SE_CRYPTO_AES_GCM_Decrypt_Append(const uint8_t *pInputBuffer, int32_t InputSize, uint8_t *pOutputBuffer, int32_t *pOutputSize);
