...
{
  /*
   * The CRC is computed with the structure without its CRC field and the length is provided to SE_LL_CRC_Calculate in 32-bit word.
   * Please note that this works only if the CRC field is kept as the last uint32_t of the SE_BootInfoTypeDef structure.
   */
  uint32_t crc = SE_LL_CRC_Calculate((uint32_t *)(x), (sizeof(SE_BootInfoTypeDef) - sizeof(uint32_t)) / sizeof(uint32_t));
...
SE_ErrorStatus SE_INFO_BootInfoAreaInit(SE_INFO_StatusTypedef *peBootInfoStatus)
{
  ...
  /* check the xBootInfo CRC. if CRC not correct then reset the structure to factory reset values - A bug or a possible threat could have changed it */
...
/**
  ...
  * @note   This function must be called in case it's not possible to read a valid BootInfoArea even after an attempted recovery .
...
SE_ErrorStatus SE_INFO_BootInfoAreaFactoryReset(void)
{
  ...
  /* Check if BootInfo Area has already been accessed via initialization function - otherwise it is not a normal sequence */
  if (eBootInfoStatus != SE_INFO_NOT_ACCESSED)
  {
    ...
    * Please note that this works only if the CRC field is kept as the last uint32_t of the SE_BootInfoTypeDef structure.
    */
    crc = SE_LL_CRC_Calculate((uint32_t *)(&boot_shared_info_empty), (sizeof(SE_BootInfoTypeDef) - sizeof(uint32_t)) / sizeof(uint32_t));
    boot_shared_info_empty.CRC32 = crc;
    e_ret_status = SE_INFO_WriteBootInfoArea(&boot_shared_info_empty, &boot_info_status);
  }
  else
  {
    /* return an error as it not normal to call FactoryReset function if BootInfo Area has not been first accessed via initialization function */
    e_ret_status = SE_ERROR;
  }
...
SE_ErrorStatus SE_INFO_ReadBootInfoArea(SE_BootInfoTypeDef *pxBootInfo, SE_INFO_StatusTypedef *peBootInfoStatus)
{
  ...
  if (SE_ERROR == SE_LL_CRC_Check(&xBootInfo))
  {
    /* update BootInfoArea Status to indicate that data can't be read as they are not valid - error should be managed by caller */
    eBootInfoStatus = SE_INFO_NOT_VALID;
    e_ret_status = SE_ERROR;
    /* xBootInfoBackUp values are not checked/used as this case shall never occur as data are initialized during initialization phase - could be a bug or an attcak */
  }
  else
  {
