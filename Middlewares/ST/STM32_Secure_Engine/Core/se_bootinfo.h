...
typedef struct
{
  uint32_t LastExecStatus;                   /*!< The last execution status set by the Secure Boot before a reset/reboot occurred */
  uint32_t LastExecError;                    /*!< The last execution error set by the Secure Boot before a reset/reboot occurred */
  uint32_t ConsecutiveBootOnErrorCounter;    /*!< Counter counting the number of consecutive boot procedures triggered by an error
                                                  (in UserApp or SB_SFU). Reset as soon as a normal boot procedure (power off/on) is performed. */
  uint32_t CRC32;                            /*!< The 32bit checksum calculated on the previous structure fields.
                                                  It is mandatory to keep this field as the last uint32_t of this structure. */
} SE_BootInfoTypeDef;
