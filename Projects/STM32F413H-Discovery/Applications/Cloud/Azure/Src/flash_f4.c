...
int FLASH_update(uint32_t dst_addr, const void *data, uint32_t size)
{
  ...
  if (copy_buffer_size> MAX_COPY_BUFFER)  copy_buffer_size = MAX_COPY_BUFFER;
  ...
  if (sector_cache_buffer != NULL)
  {
    ...
    do {
      ...
      if (copy_size > copy_buffer_size ) copy_size= copy_buffer_size;
      memcpy(sector_cache, (void *) fl_addr, copy_size);  
      /* allocatd buffer is too small */
      if (len > copy_size ) return rc;
