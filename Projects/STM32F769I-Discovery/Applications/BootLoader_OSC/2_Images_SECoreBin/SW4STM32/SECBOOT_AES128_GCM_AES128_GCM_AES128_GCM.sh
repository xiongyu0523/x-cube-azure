...
if [ $ret == 0 ]; then
  ...
  ret=$?
  if [ $ret == 0 ]; then 
    command=$cmd" "$prepareimage" pack -k "$oemkey"  -r 72 -v "$version" -n "$nonce" -f "$sfu" -t "$sign" "$sfb" -o 1024"
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
      command=$cmd" "$prepareimage" header -k  "$oemkey" -r 72 -v "$version"  -n "$nonce" -f "$sfu" -t "$sign" -o 1024 "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
