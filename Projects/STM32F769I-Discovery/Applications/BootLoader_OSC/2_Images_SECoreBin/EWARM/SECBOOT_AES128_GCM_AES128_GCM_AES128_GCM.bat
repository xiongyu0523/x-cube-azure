...
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% pack -k  %oemkey%  -r 72 -v %version% -n %nonce% -f %sfu% -t %sign% %sfb% -o 1024 >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% header -k  %oemkey%  -r 72 -v %version% -n %nonce% -f %sfu% -t %sign% -o 1024 %headerbin%  >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
